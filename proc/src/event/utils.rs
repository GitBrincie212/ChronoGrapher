...
impl Payload {
    pub(crate) fn anonymize(self) -> Self {
        match self {
            Payload::NamedFields(fields) => {
                Payload::UnnamedFields(fields.into_iter().map(|x| x.ty).collect())
            }
...
pub fn parse_individual_event(
    ...
    if args.inline.is_present() && args.payload_name_override.is_some() {
        return Err(syn::Error::new(
            ...
            "Cannot override payload name while inlining it",
        ));
    }
    ...
    let payload_lt = first_lt
        .cloned()
        .unwrap_or_else(|| LifetimeParam::new(Lifetime::new("'a", proc_macro2::Span::call_site())));
    ...
    let other_generics_impl = event_generics
        .params
        .iter()
        .filter(|param| param.as_lifetime_param().is_none())
        .collect::<Punctuated<_, Comma>>();

    let other_generics = other_generics_impl
        .iter()
    ...
    match event_fields {
        ...
        syn::Fields::Unnamed(fields) if args.inline.is_present() => {
            return Err(syn::Error::new_spanned(
                fields,
                "Cannot use the inline attribute for tuple-based payloads",
            ));
        }
