...
const UNKNOWN_ATTRIBUTE_PARAM_ERR: &str =
    "Unknown attribute parameter, use either \"thread_count\" or \"start_before\"";
const FUNCTION_NOT_MAIN_ERR: &str =
    "Expected function to be a main function and nto something else";
const UNEXPECTED_SELF_ARG_ERR: &str = "Unexpected &self / &mut self used in main function";
const SIMPLE_IDENTIFIER_FOR_SCHEDULER_ERR: &str =
    "Expected a simple identifier as argument name for the scheduler";
const EXPECTED_AT_LEAST_ONE_SCHEDULER_ERR: &str =
    "Expected at least one argument for the scheduler to initialize";
const UNEXPECTED_RETURN_TYPE_ERR: &str =
    "Expected either () or a Return<T, E> as return type but got something else";
const START_BEFORE_FLAG_ERR: &str = "start_before parameter has to used as a flag only";
const UNKNOWN_ATTRIBUTE_FLAG_ERR: &str =
    "Unknown attribute flag, did you mean to use \"start_before\"?";
const THREAD_COUNT_SPECIFY_ERR: &str = "Already specified the thread count parameter";
const THREAD_COUNT_INT_ERR: &str = "Thread count parameter must be an integer literal";
