...
Though they are not limited to one event, they can run their own *"Side Effects"* per event,
which is a fancy way of saying different code that reacts for every event.

In addition, they aren't just event listeners but extension points for a Task, allowing you to embed additional state per
Task instance and access it at runtime. They can also communicate with each other and so on.
