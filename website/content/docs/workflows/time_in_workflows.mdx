...
async fn APIRequestTask(ctx: &TaskFrameContext) -> Result<(), MyErrors> {
    // <...>
}
```
...
Timeouts can also be calculated dynamically via a custom function, each time the timeout is about to start the function
is invoked to calculate the corresponding duration:
<RenderProgrammingLanguageBased target_name={"Rust"}>
```rust title="src/main.rs" lineNumbers=4
#[task(schedule = every!(1s)))]
#[workflow(
    // [!code highlight]
    timeout(custom = |_| Duration::from_secs(5)),
    fallback(MyFallbackTaskFrame),
    retry(3, 2s)
)]
async fn APIRequestTask(ctx: &TaskFrameContext) -> Result<(), MyErrors> {
...
While ChronoGrapher automatically handles the error value for timeouts, it includes another parameter to control the error
when a timeout does occur via ``on_timeout``:
<RenderProgrammingLanguageBased target_name={"Rust"}>
```rust title="src/main.rs" lineNumbers=4
#[task(schedule = every!(1s)))]
#[workflow(
    // <...>
    timeout(5s, on_timeout = constant(MyErrors::TimeoutVariant)), // Constant-value
    timeout(5s, on_timeout = function(|| MyErrors::TimeoutVariant)), // Function-based
    // <...>
)]
async fn APIRequestTask(ctx: &TaskFrameContext) -> Result<(), MyErrors> {
    // <...>
...
We may want to delay the execution of part of our workflow or the entire (though the latter is much rare)
...
Just like ``timeout``, we can also use a custom function to calculate dynamically the delay:
...
#[workflow(
    timeout(6s),
    fallback(myfallback),
    delay(custom = |_| Duration::from_secs(1)),
    retry(3, 2s),
)]
async fn APIRequestTask(ctx: &TaskFrameContext) -> Result<(), MyErrors> {
    // <...>
...
As far as it goes for ``delay``, it is used only for delaying some parts by a specified constant or dynamically computed duration.
...
``delay`` is less common but used for delaying some workflow parts by a constant or dynamically computed duration.