...
#[taskframe]
// [!code ++]
async fn MyFallbackTaskFrame(ctx: &TaskFrameContext, error: &MyErrors) -> Result<(), MyErrors> {
    // [!code ++]
    ...
    match error {
        ...
        err => {
            ...
            // [!code ++]
            Err(err.clone()) // We have to clone the value or use an Arc<T> if expensive
            // [!code ++]
        }
        // [!code ++]
    }
...
```
<Callout type={"idea"} title={"The Clone For Our Error"}>
    Its often an anti-pattern to throw different errors from the fallback, usually its best to throw the same error but
    since we borrow it, we can't simply throw it back easily and hence we need to clone.

    If your error type is expensive to clone then consider the usage of ``Arc<T>``.
</Callout>
</RenderProgrammingLanguageBased>
When a fallback is executed, it executes our Task logic primarily, then if it fails, we share the error with
the fallback to resolve. Now when our code runs, in a realistic environment we may see:
```zsh title="terminal"
...
<Callout type={"idea"} title={"Clever Retrying"}>
    ...
    A pro tip is to use the ``when`` parameter on retries which is covered in [Advanced Retries](/workflows/advanced_retries#retry-error-filtering).
</Callout>
