...
- ``# Semantics`` Lists how the struct is meant to be used and what it does (can be skipped if above headers or explanations are sufficient). **(RECOMMENDED)**
- ``# Method(s)`` Lists and briefly describes the methods (only functions, no getters/setters, if any). **(REQUIRED)**
...
- ``# Accessing/Modifying Field(s)`` Lists the fields which you can access and/or modify (via setters/getters or via just normal Rust fields, if any). **(REQUIRED)**
- ``# Trait Implementation(s)`` Lists the various implementations the struct implements (not including blanket implementations, if any). **(REQUIRED)**
...
- ``# Semantics`` Lists how the enum is meant, what each variant is best suited to and what it does(can be skipped if above headers or explanations are sufficient). **(RECOMMENDED)**
- ``# Variant(s)`` Lists and briefly describes the variants of this enum. **(REQUIRED)**
- ``# Method(s)`` Lists and briefly describes the methods (only functions, no getters/setters, if any). **(REQUIRED)**
- ``# Constructor(s)`` Lists the various constructors you can use (including builders, normal rust initialization and via the Default trait) for each variant, if any. **(REQUIRED)**
- ``# Accessing/Modifying Field(s)`` Lists the fields which you can access and/or modify (via setters/getters or via just normal Rust fields, if any). **(REQUIRED)**
- ``# Trait Implementation(s)`` Lists the various implementations the enum implements (not including blanket implementations, if any). **(REQUIRED)**
- ``# Generic(s)`` Describes the generics which the enum may have (don't include it if there aren't any). **(RECOMMENDED)**

**Trait Sections**
- ``# Semantics`` Lists how the trait is meant to be implemented, used and what it does(can be skipped if above headers or explanations are sufficient). **(RECOMMENDED)**
- ``# Required Method(s)`` Lists the various required methods and what they are meant to do, if any. **(REQUIRED)**
- ``# Optional Method(s)`` Lists the various optional methods, what they are meant to do and their default behavior, if any. **(RECOMMENDED)**
...
**Method Sections**
- ``# Semantics`` Lists how the method is meant to be used and what it does(can be skipped if above headers or explanations are sufficient). **(RECOMMENDED)**
...
- ``# Error(s)`` Lists the various errors which may appear, under what conditions each error appears in, if any. **(REQUIRED)**
- ``# Panics`` Lists the various reasons the method may panic, under what circumstances and why, if any. **(REQUIRED)**
...
- ``# External Interactions`` Explains how this macro behaves when combined
...
- ``# Invocation Syntax`` Shows the exact syntax for calling the macro, including all valid forms and what kind of inputs it accepts. **(REQUIRED)**
