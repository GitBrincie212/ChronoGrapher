...
We recommend installing [thiserror](https://docs.rs/thiserror/latest/), this allows for better error handling,
...
their own error approach or any other crate or utility for error handling.
...
#[derive(Error, Debug)]
pub enum MyErrors {} // For now empty, we will fill it later
...
<Callout type={"info"} title={"The \"scheduler\" argument"}>
    ...
    However, more importantly, note ``DefaultScheduler`` is a type alias to a ``Scheduler`` with default configurationsm
    it accepts a generic for the type of task errors allowed.
</Callout>
</RenderProgrammingLanguageBased>
<br />
# What Are Tasks?
...
the former is **the code we want to run**, which can be database operations, API requests, maintenance and other computation.
