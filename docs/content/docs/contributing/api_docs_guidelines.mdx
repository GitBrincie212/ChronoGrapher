...
- ``# Policy Semantics`` Describes the semantics of the strategy / policy, how rescheduling and overlapping tasks are handled. **(REQUIRED)**
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of the ScheduleStrategy in some way. **(RECOMMENDED)**
