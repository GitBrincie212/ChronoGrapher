...
### **Prelude Section**
The ``Prelude`` module re-exports the most common components from the core library. With the help of the prelude, developers can use the ChronoGrapher library with reduced import boilerplate, just by a single `use` statement.
> The prelude includes core task and scheduler types, common task frames, schedule types (interval/calendar/cron), hooks/events, and retry/dependency helpers.

We will explain **schedule types**, **task frames** and other modules more in depth in the following API Guidelines.
<Callout type={"info"} title={"Side Note"}>
  For best development experience, developers should start with basics of the ChronoGrapher using the Prelude. 
</Callout>
In the current state of the ChronoGrapher Core, we have several components out of the box when you first add the cargo package. It contains top-level modules like ``Scheduler``, ``Utils``, ``Task``, and most importantly ``Prelude``. To use the prelude exports, add:
```rust
...
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of the TaskFrame in some way. **(RECOMMENDED)**
...
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of this TaskHook in some way. **(RECOMMENDED)**
...
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of the SchedulerEngine in some way. **(RECOMMENDED)**
...
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of the SchedulerTaskDispatcher in some way. **(RECOMMENDED)**
...
- ``# Supported TaskHook(s)`` Describes what kind of TaskHooks are supported which change the behavior of the SchedulerTaskStore in some way. **(RECOMMENDED)**
...
However, it is best to discuss this with the maintainer of the project (McBrincie212) beforehand, to reach a conclusion,
introducing new headers could mean updating this spec and previous other API docs.