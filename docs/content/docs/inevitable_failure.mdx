...
Thus, let's introduce a more "realistic" scenario via a new Task (where the function is a TaskFrame):
...
<Callout type={"warning"} title={"Disclaimer About The Script"}>
...
```rust title="src/main.rs" lineNumbers=15
    ...
    for i in 0..5 {
        result2 = fallible_part2();
        if (result2.is_ok()) break;
        tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    }
...
and intuitive with its workflow model (simple primitives → full-on workflows)