...
```
The delay per retries can be configured via a **Backoff Strategy**, Backoff strategies controls how the delay scales
...
| fallback(WORKFLOW)  | fallback(mytask_fallback) | If the inner workflow fails, it executes the ``WORKFLOW`` workflow |
