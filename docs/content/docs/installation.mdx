...
/>
Where a Task has 2 separate parts (Business Logic, Scheduling Logic). Then the Task interacts with the ``Scheduler``
to schedule at a specific time(more specifically execute at **9:00 AM**).

Once the time is reached, our Scheduler alerts our Task **to start executing the business logic**, then once our
...
example. Tasks are composed of TaskFrames (business logic), TaskSchedules (scheduling logic),
