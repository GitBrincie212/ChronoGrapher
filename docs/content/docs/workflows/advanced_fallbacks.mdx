...
```rust lineNumbers=8
type TargetFallbackError = RetriableTaskFrameError<FallbackTaskFrameError<TimeoutTaskFrameError<MyError>>>>;
...
async fn MyFallbackTaskFrame(ctx: &TaskContext) -> Result<(), MyErrors> {
    // [!code highlight]
    if let Some(error) = ctx.get_shared::<SharedFallbackError<TargetFallbackError>>>().await {
        // In real-world scenarios you handle these errors more gracefully
        return **match error.0 {
            // ...Handling The TimeoutTaskFrameError<MyError> Gracefully (both timeouts and MyError)...
        };
    }
    Ok(())
// [!code ++]
...
While this approach provides structured error matching, the problem is it requires a static workflow configuration and updating
multiple points throughout our program. Alternatively one can do the following:
```rust lineNumbers=8
if let Some(error) = ctx.get_shared::<SharedFallbackError<Box<dyn TaskError>>().await {
    // ...Handling downcasts and peeling off new type errors...
}
```
