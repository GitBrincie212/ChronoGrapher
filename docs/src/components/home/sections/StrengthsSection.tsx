...
async fn main() {
  ...
  return (
    ...
    >
      <div className={"w-full h-fit flex"}>{/* CODE HERE */}</div>
    </div>
  );
}
...
const strengthsList = [
  {
    ...
    description:
      "One unified platform connected across your favourite programming languages, No more glue-code, no more leaky patterns and no more brittleness. Everything as it's meant to be.",
    icon: (
      <path
        fill="none"
        stroke="url(#strengths-gradient)"
        strokeLinecap="round"
  ...
  },
  {
    ...
    description:
      "Swap every component without incurring massive rewrites, clocks, dispatchers, stores and even the engine. ChronoGrapher adapts to your infrastructure, not the other way around.",
    icon: (
      <path
        fill="none"
        stroke="url(#strengths-gradient)"
  ...
  },
  {
    ...
    description:
      "Configure your level of durability to persist state automatically, resume workflows where they left off. Crashes, restarts, and infrastructure failures don't mean lost work.",
    icon: (
      <path
        fill="none"
        stroke="url(#strengths-gradient)"
  ...
  },
  {
    ...
    description:
      "Zero-cost abstraction and lightning-fast tickless scheduling algorithms allow millions of tasks in throughput with minimal overhead. Configurable for all your blazing desires.",
    icon: (
      <path
        fill="none"
        stroke="url(#strengths-gradient)"
  ...
  },
  {
    ...
    description:
      "From home laptops to full distributed clusters managed with Kubernetes, monitored with Prometheus and Scheduled with ChronoGrapher, make enterprise scale dreams possible.",
    icon: (
      <g
        fill="none"
        stroke="url(#strengths-gradient)"
...
function StrengthToggle(props: {
  setStrengthIdx: React.Dispatch<React.SetStateAction<number>>;
  strengthIdx: number;
  idx: number;
}) {
  return (
    <button
      type="button"
      className={`cursor-pointer h-full rounded-full transition-all duration-300 ring-offset-2 ring-1 py-2
                ring-offset-fd-background ${
                  props.strengthIdx === props.idx
...
export default function StrengthsSection() {
  ...
  useEffect(() => {
    ...
    strengthsContainer.current.style.setProperty(
      "--stop-color-start",
      strengthsList[strengthIdx].startColor,
    );
    strengthsContainer.current.style.setProperty(
      "--stop-color-end",
      strengthsList[strengthIdx].endColor,
    );
  }, [strengthIdx]);
  ...
  return (
    <div className={"w-screen flex justify-between h-96 mt-18 px-32 gap-32"}>
      <div
        className={"w-fit flex flex-col justify-center h-full gap-2"}
        ref={strengthsContainer}
      >
        <div className={"flex items-center gap-2"}>
          ...
          >
            <div className={"w-full h-full bg-fd-muted rounded-md"}>
              <div
                className={
                  "w-full h-full flex justify-center items-center text-5xl p-2"
                }
              >
                ...
                >
                  <defs>
                    ...
                    >
                      <stop
                        style={{ stopColor: `var(--stop-color-start)` }}
                        offset="0%"
                      />
                      <stop
                        style={{ stopColor: `var(--stop-color-end)` }}
                        offset="100%"
                      />
                    </linearGradient>
                  </defs>
          ...
          <div className={"text-2xl font-semibold"}>
            {strengthsList[strengthIdx].title}
          </div>
        </div>
